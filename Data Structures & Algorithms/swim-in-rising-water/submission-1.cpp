    ...
    int swimInWater(vector<vector<int>>& grid) {
        ...
        while (!minHeap.empty()){
            ...
            for (const auto& dir : directions){
                ...
                minHeap.push({max(t, grid[neiR][neiC]), neiR, neiC}); // because if the time is smaller, then we will have to wait for 
                // water to rise up to that level. if level is smaller, then in the path to here we still needed that time regardless.
